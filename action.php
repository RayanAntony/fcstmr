...
{  
	if($_REQUEST["action"] == "login")
	{
		if($_REQUEST["username"]!='' && $_REQUEST["password"]!=''):
			...
			$login_check = $object->check_user_exists("SELECT * FROM user WHERE username='".$username."' AND password='".$password."'");
			if($login_check!=''):
	...
	{  
		echo json_encode($object->get_data_in_table("SELECT * FROM fcstmr"));  
	}
	...
	{  
		if($_REQUEST["edit_fcstmraid"]!=''):
			echo json_encode($object->get_data_in_table("SELECT * FROM fcstmr WHERE aid='".$_REQUEST["edit_fcstmraid"]."'"));
		endif;
	}
